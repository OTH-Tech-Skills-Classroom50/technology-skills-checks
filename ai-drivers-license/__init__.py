...
def is_valid_pdf():
    ...
    pdf_path = find_pdf()
    _doc, _text, err = ca.load(pdf_path)
    if err:
...
def correct_course():
    ...
    _doc, text, _err = ca.load(find_pdf())
...
def integrity_review():
    ...
    pdf_path = find_pdf()
    doc, text, _err = ca.load(pdf_path)
    info = ca.extract_display_info(text)
    signals = ca.detect_tamper_signals(pdf_path, doc)
    ...
    # here (and picked up later by the teacher-side review script) for a
    # human to judge, never used to block a student's score. See
    # certificate_analysis.detect_tamper_signals' docstring for why.
    check50.data(name=info["name"], tamper_signals=signals)
